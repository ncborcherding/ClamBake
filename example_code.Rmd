...
plot.interval.EE <- function(cage.data,
    ...
    indep.vairables <- pre.plot.dat[,c("Feed", "Drink", "Heat", "Weight", "Interval_num")]
    indep.vairables <- indep.vairables %>%
    ...
    indep.vairables$heat.mean <- rowMeans(as.matrix(indep.vairables[,c("heat.spline", "heat.gam", "heat.glm")]))
    ...
    EE.summary <- indep.vairables %>%
      ...
      summarise(across(c(3,7:14), mean)) %>%
      as.data.frame()
...
plot.interval.EE(dat.cre, 
                 by.proportion = FALSE) + 
  ylim(0,40)
ggsave("dat.cre.pdf", height = 3, width = 5)
plot.interval.EE(dat.cre, 
                 by.proportion = TRUE)
ggsave("dat.cre.prop.pdf", height = 3, width = 5)

plot.interval.EE(dat.flox, 
                 by.proportion = FALSE)+ 
  ylim(0,40)
ggsave("dat.flox.pdf", height = 3, width = 5)
plot.interval.EE(dat.flox, 
                 by.proportion = TRUE)
ggsave("dat.flox.prop.pdf", height = 3, width = 5)
```

```{r}
cre.data <- bind_rows(dat.cre[[1]])
cre.data <- cre.data %>%
       mutate("Ambulation" = Xamb + Yamb) 
...
fd <- data.frame("Ambulation" = flox.data$Ambulation, "Interval" = flox.data$Interval_num)
fd[,3] <- "Flox"
colnames(fd)[3] <- "Condition"

cd <- data.frame("Ambulation" = cre.data$Ambulation, "Interval" = cre.data$Interval_num)
cd[,3] <- "Cre"
colnames(cd)[3] <- "Condition"
